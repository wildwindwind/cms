...
<div class="swap">
	<table class="list-box">
		...
		<tr class="box-title">
			<th colspan="20">
				<span class="list-icon fl">流程实例</span>
	  		</th>
		</tr>	
		<tr>
			<th>流程名称</th>
		...
		<tr <#if (item_index%2==0)>class='t1'<#else>class='t2'</#if>>
			...
			<td>${item.creator}</td>
			<td>${(item.orderState == 0)?string('已结束','运行中')}</td>
			<td>
				<a href="${ctx}/admin/snaker/process/display.do?processId=${item.processId}&orderId=${item.id}">查看流程图</a>
			</td>
		</tr>
		</#list>
	</table><!-- end table -->
